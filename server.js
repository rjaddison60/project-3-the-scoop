// Project 3 The Scoop project submitted by RJ Addison
// rjaddison.rja@gmail.
...
const routes = {
  ...
  // Update routes to include comment code
  '/comments': {
    'POST': createComment
  },
  '/comments/:id': {
...
// Here is where the code begins for the assignment
// Create new comments
...
function updateComment(url, request) {
  console.log(url, request, 'update comment');
  const id = Number(url.split('/').filter(segment => segment)[1]);
...
function downvoteComment(url, request) {
  ...
  if (savedComment && database.users[username]) {
    savedSaved = downvote(savedComment, username);
